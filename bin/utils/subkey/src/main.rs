...
use sp_core::{
    ...
    sr25519, Bytes, Pair, Public, H256,
};
use sp_runtime::{
    generic::Era,
...
fn get_usage() -> String {
    let networks = Ss58AddressFormat::all()
        .iter()
        .cloned()
        .map(String::from)
        .join("/");
    ...
    PublicOf<C>: PublicT,
{
    ...
    let maybe_network: Option<Ss58AddressFormat> =
        match matches.value_of("network").map(|network| {
            network.try_into().map_err(|_| {
                Error::Static("Invalid network name. See --help for available networks.")
            })
        }) {
    ...
    let output: OutputType = match matches.value_of("output").map(TryInto::try_into) {
        Some(Err(_)) => {
            return Err(Error::Static(
                "Invalid output name. See --help for available outputs.",
            ))
        }
        Some(Ok(v)) => v,
    ...
    match matches.subcommand() {
        ...
        ("generate-node-key", Some(matches)) => {
            let file = matches
                .value_of("file")
                .ok_or(Error::Static("Output file name is required"))?;
        ...
        ("inspect-node-key", Some(matches)) => {
            let file = matches
                .value_of("file")
                .ok_or(Error::Static("Input file name is required"))?;
        ...
        ("insert", Some(matches)) => {
            ...
            let key_type = matches
                .value_of("key-type")
                .ok_or(Error::Static("Key type id is required"))?;

            // Just checking
            let _key_type_id = sp_core::crypto::KeyTypeId::try_from(key_type).map_err(|_| {
                Error::Static(
        ...
        ("localstorage-get", Some(matches)) => {
            ...
            let key = matches
                .value_of("key")
                .ok_or(Error::Static("Key is required"))?;
        ...
        ("localstorage-set", Some(matches)) => {
            ...
            let value = matches
                .value_of("value")
                .ok_or(Error::Static("Value is required"))?;
...
fn read_required_parameter<T: FromStr>(matches: &ArgMatches, name: &str) -> Result<T, Error>
where
    <T as FromStr>::Err: std::fmt::Debug,
{
    ...
    str::parse::<T>(str_value).map_err(|_| {
        Error::Formatted(format!(
            "Invalid `{}' parameter; expecting an integer.",
            name
        ))
    })
}
...
mod tests {
        ...
        PublicOf<CryptoType>: PublicT,
    {
        ...
        let signature =
            do_sign::<CryptoType>(&seed, message.clone(), password).expect("signing failed");

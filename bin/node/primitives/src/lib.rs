...
pub trait AssetTrait<AssetId, AccountId, Balance, Cost, Income> {
    ...
    fn asset_create(
        symbol: Vec<u8>,
        precision: u16,
    ) -> Result<(AssetId, Token<Balance>), Self::Error>;
...
impl<AssetId, AccountId, Balance, Cost, Income>
    AssetTrait<AssetId, AccountId, Balance, Cost, Income> for ()
where
    AssetId: Default,
    AccountId: Default,
    Balance: Default,
    ...
    Income: Default,
{
    ...
    fn asset_create(_: Vec<u8>, _: u16) -> Result<(AssetId, Token<Balance>), Self::Error> {
        Ok(Default::default())
    }
    ...
    fn asset_id_exists(_: &AccountId, _: &[u8], _: u16) -> Option<TokenSymbol> {
        Default::default()
    }

    fn token_exists(_: TokenSymbol) -> bool {
        Default::default()
    }

    fn get_account_asset(_: TokenSymbol, _: &AccountId) -> AccountAsset<Balance, Cost, Income> {
        Default::default()
    }

    fn get_token(_: TokenSymbol) -> Token<Balance> {
        Default::default()
    }
...
impl<TokenSymbol, ER: Default> FetchConvertPrice<TokenSymbol, ER> for () {
    fn fetch_convert_price(_: TokenSymbol) -> ER {
        Default::default()
    }
}
...
pub trait BridgeAssetFrom<AccountId, Precision, Balance> {
    fn bridge_asset_from(
        target: AccountId,
        bridge_asset: BridgeAssetBalance<AccountId, Precision, Balance>,
    );
}
...
impl<A, P, B> BridgeAssetTo<A, P, B> for () {
    type Error = core::convert::Infallible;
    fn bridge_asset_to(_: Vec<u8>, _: BridgeAssetBalance<A, P, B>) -> Result<(), Self::Error> {
        Ok(())
    }
    fn redeem(_: TokenSymbol, _: B, _: Vec<u8>) -> Result<(), Self::Error> {
        Ok(())
    }
    fn stake(_: TokenSymbol, _: B, _: Vec<u8>) -> Result<(), Self::Error> {
        Ok(())
    }
    fn unstake(_: TokenSymbol, _: B, _: Vec<u8>) -> Result<(), Self::Error> {
        Ok(())
    }
}
...
impl<A, B> AssetReward<A, B> for () {
    type Output = ();
    type Error = core::convert::Infallible;
    fn set_asset_reward(_: A, _: B) -> Result<Self::Output, Self::Error> {
        Ok(())
    }
}

...
use rococo_runtime::constants::currency::BNCS as RCO;
...
fn rococo_staging_testnet_config_genesis() -> rococo::GenesisConfig {
	// stash, controller, session-key
	// generated with secret:
	// for i in 1 2 3 4 ; do for j in stash controller; do subkey inspect "$secret"/fir/$j/$i; done; done
	// and
...
pub fn rococo_staging_testnet_config() -> RococoChainSpec {
	...
	RococoChainSpec::from_genesis(
		...
		boot_nodes,
		Some(TelemetryEndpoints::new(vec![(ROCOCO_STAGING_TELEMETRY_URL.to_string(), 0)])
			.expect("Staging telemetry url is valid; qed")),
		None,
		None,
		Default::default(),
...
pub fn rococo_testnet_genesis(
	...
	rococo::GenesisConfig {
		frame_system: Some(rococo::SystemConfig {
			code: rococo::wasm_binary_unwrap().to_vec(),
			changes_trie_config: Default::default(),
		}),
		...
		pallet_indices: Some(rococo::IndicesConfig {
			indices: vec![],
		}),
		pallet_session: Some(rococo::SessionConfig {
			keys: initial_authorities.iter().map(|x| {
				(x.0.clone(), x.0.clone(), rococo_session_keys(
					x.2.clone(),
					x.3.clone(),
					x.4.clone(),
					x.5.clone(),
		...
		pallet_staking: Some(rococo::StakingConfig {
			...
			minimum_validator_count: initial_authorities.len() as u32,
			stakers: initial_authorities.iter().map(|x| {
				(x.0.clone(), x.1.clone(), STASH, rococo::StakerStatus::Validator)
			}).collect(),
			invulnerables: initial_authorities.iter().map(|x| x.0.clone()).collect(),
			slash_reward_fraction: Perbill::from_percent(10),
			.. Default::default()
		}),
		pallet_democracy: Some(rococo::DemocracyConfig::default()),
		pallet_elections_phragmen: Some(rococo::ElectionsConfig {
			members: endowed_accounts.iter()
				.take((num_endowed_accounts + 1) / 2)
				.cloned()
				.map(|member| (member, STASH))
		...
		pallet_collective_Instance1: Some(rococo::CouncilConfig::default()),
		pallet_collective_Instance2: Some(rococo::TechnicalCommitteeConfig {
			members: endowed_accounts.iter()
				.take((num_endowed_accounts + 1) / 2)
				.cloned()
				.collect(),
		...
		pallet_sudo: Some(rococo::SudoConfig {
			key: root_key.clone(),
		}),
		pallet_babe: Some(rococo::BabeConfig {
			authorities: vec![],
		}),
		pallet_im_online: Some(rococo::ImOnlineConfig {
			keys: vec![],
		}),
		pallet_authority_discovery: Some(rococo::AuthorityDiscoveryConfig {
			keys: vec![],
		}),
		pallet_grandpa: Some(rococo::GrandpaConfig {
			authorities: vec![],
		}),
		pallet_membership_Instance1: Some(Default::default()),
		pallet_treasury: Some(Default::default()),
		pallet_society: Some(rococo::SocietyConfig {
			members: endowed_accounts.iter()
				.take((num_endowed_accounts + 1) / 2)
				.cloned()
				.collect(),
		...
		pallet_vesting: Some(Default::default()),
		brml_assets: Some(rococo::AssetsConfig {
			account_assets: vec![],
			next_asset_id: 9u32, // start from 7, [0..6] has been reserved
			token_details: vec![],
			prices: vec![],
		}),
		brml_convert: Some(rococo::ConvertConfig {
			convert_price: vec![
				(TokenSymbol::DOT, DOLLARS / 100),
				(TokenSymbol::KSM, DOLLARS / 100),
				(TokenSymbol::EOS, DOLLARS / 100),
		...
		brml_bridge_eos: Some(rococo::BridgeEosConfig {
			bridge_contract_account: (b"bifrostcross".to_vec(), 2),
			notary_keys: initial_authorities.iter().map(|x| x.0.clone()).collect::<Vec<_>>(),
			// alice and bob have the privilege to sign cross transaction
			cross_chain_privilege: [(root_key.clone(), true)].iter().cloned().collect::<Vec<_>>(),
		...
		brml_bridge_iost: Some(rococo::BridgeIostConfig {
			bridge_contract_account: (b"lispczz4".to_vec(), 1),
			notary_keys: initial_authorities.iter().map(|x| x.0.clone()).collect::<Vec<_>>(),
			// alice and bob have the privilege to sign cross transaction
			cross_chain_privilege: [(root_key.clone(), true)].iter().cloned().collect::<Vec<_>>(),
		...
		brml_voucher: {
			if let Some(vouchers) = initialize_all_vouchers() {
				Some(rococo::VoucherConfig { voucher: vouchers })
			} else {
				None
			}
		},
		// brml_swap: initialize_swap_module(root_key),
		brml_swap: None,
		parachain_info: Some(rococo::ParachainInfoConfig { parachain_id: 200u32.into() }),
	}
}
...
fn rococo_development_config_genesis(_wasm_binary: &[u8]) -> rococo::GenesisConfig {
	rococo_testnet_genesis(
		vec![get_authority_keys_from_seed("Alice")],
		get_account_id_from_seed::<sr25519::Public>("Alice"),
		None,
...
fn rococo_local_testnet_genesis(_wasm_binary: &[u8]) -> rococo::GenesisConfig {
	rococo_testnet_genesis(
		vec![
			get_authority_keys_from_seed("Alice"),
			get_authority_keys_from_seed("Bob"),
...
pub fn rococo_chainspec_config() -> RococoChainSpec {
	let properties = {
		let mut props = serde_json::Map::new();

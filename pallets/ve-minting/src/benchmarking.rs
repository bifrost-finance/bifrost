...
benchmarks! {
	...
	create_lock {
		...
		T::MultiCurrency::deposit(CurrencyId::Native(TokenSymbol::BNC), &test_account, BalanceOf::<T>::unique_saturated_from(500000000000000000u128))?;
		T::MultiCurrency::deposit(CurrencyId::VToken(TokenSymbol::BNC), &test_account, BalanceOf::<T>::unique_saturated_from(500000000000000000u128))?;
		let rewards = vec![(CurrencyId::Native(TokenSymbol::BNC), 1000u32.into())];
	...
	notify_rewards {
		...
		let rewards = vec![(CurrencyId::Native(TokenSymbol::BNC), 1000u32.into())];
		T::MultiCurrency::deposit(CurrencyId::Native(TokenSymbol::BNC), &account("seed",1,1), 10000u32.into())?;
	}: _<T::RuntimeOrigin>(origin,account("seed",1,1),Some((7 * 86400 / 12u32).into()),rewards)

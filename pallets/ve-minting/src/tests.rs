...
fn notify_reward_amount() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(VeMinting::increase_amount(
...
fn create_lock_to_withdraw() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				VeMinting::balance_of_at(&BOB, System::block_number()),
				Ok(87397254003200)
			);
			assert_eq!(
				VeMinting::balance_of_at(&BOB, System::block_number() - 10),
				Ok(0)
			);
			assert_eq!(VeMinting::balance_of_at(&BOB, 0), Ok(0));
			assert_eq!(
				VeMinting::balance_of(&BOB, Some(System::block_number() - 10)),
				Ok(0)
			);
			assert_eq!(
				VeMinting::total_supply(System::block_number()),
			...
			assert_noop!(
				VeMinting::withdraw(RuntimeOrigin::signed(BOB)),
				Error::<Runtime>::Expired
			);
			assert_eq!(
				VeMinting::total_supply(System::block_number()),
...
fn overflow() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				VeMinting::balance_of(&BOB, Some(77001)),
				Ok(100_000_000_000_000)
			);
			assert_eq!(
				VeMinting::total_supply(System::block_number()),

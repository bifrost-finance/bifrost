...
pub mod v3 {
	...
	pub fn pre_migrate<T: Config>() -> Result<(), &'static str> {
		...
		log::info!("total {:#?} reward speed items need to migrate", reward_speed_count);

		let last_accrued_timestamp = LastAccruedTimestamp::<T>::get();
		log::info!("LastAccruedTimestamp: {:#?} is about to move.", last_accrued_timestamp);
		...
		log::info!("👜 lend-market v3 migration passes PRE migrate checks ✅",);
	...
	pub fn migrate<T: Config>() -> Weight {
		if StorageVersion::<T>::get() == crate::Versions::V2 {
			log::info!("migrating lend-market to Versions::V3",);
			...
			log::info!("👜 completed lend-market migration to Versions::V3",);
	...
	pub fn post_migrate<T: Config>() -> Result<(), &'static str> {
		...
		let reward_speed_count = MarketRewardSpeed::<T>::iter().count();
		log::info!("total {:#?} reward speed items remains after migrate", reward_speed_count);

		let last_accrued_timestamp = LastAccruedTimestamp::<T>::get();
		log::info!("LastAccruedTimestamp: {:#?} after migrate.", last_accrued_timestamp);
		...
		log::info!("👜 lend-market v3 migration passes POST migrate checks ✅",);

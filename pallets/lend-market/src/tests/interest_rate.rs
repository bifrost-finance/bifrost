...
fn utilization_rate_works() {
	// 50% borrow
	assert_eq!(
		LendMarket::calc_utilization_ratio(1, 1, 0).unwrap(),
		Ratio::from_percent(50)
	);
	assert_eq!(
		LendMarket::calc_utilization_ratio(100, 100, 0).unwrap(),
		Ratio::from_percent(50)
	);
	// no borrow
	assert_eq!(
		LendMarket::calc_utilization_ratio(1, 0, 0).unwrap(),
		Ratio::zero()
	);
	// full borrow
	assert_eq!(
		LendMarket::calc_utilization_ratio(0, 1, 0).unwrap(),

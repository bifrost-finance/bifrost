...
fn exceeded_supply_cap() {
	new_test_ext().execute_with(|| {
		Assets::mint(RuntimeOrigin::signed(ALICE), DOT.into(), ALICE, million_unit(1001)).unwrap();
...
fn repay_borrow_all_no_underflow() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Loans::borrow(RuntimeOrigin::signed(ALICE), KSM, 10_u128.pow(7)));
...
fn redeem_all_should_be_accurate() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(Loans::exchange_rate(KSM), Rate::from_inner(20000000036387000));

...
use super::types::MantaCall;
use crate::{
	agents::{MantaCurrencyId, MantaParachainStakingCall, MantaXtokensCall},
	pallet::{Error, Event},
	primitives::{
...
/// StakingAgent implementation for Manta
pub struct MantaAgent<T>(PhantomData<T>);

impl<T> MantaAgent<T> {
	pub fn new() -> Self {
		MantaAgent(PhantomData::<T>)
	}
}
	...
	> for MantaAgent<T>
{
	...
	fn bond(
		...
		let call = MantaCall::ParachainStaking(MantaParachainStakingCall::<T>::Delegate(
	...
	fn bond_extra(
		...
		let call = MantaCall::ParachainStaking(MantaParachainStakingCall::<T>::DelegatorBondMore(
			validator_account,
			amount,
		));
	...
	fn unbond(
		...
		// Construct xcm message.
		let validator_account = Pallet::<T>::multilocation_to_account(&collator)?;
		let call = MantaCall::ParachainStaking(
			MantaParachainStakingCall::<T>::ScheduleDelegatorBondLess(validator_account, amount),
		);
	...
	fn liquidize(
		...
		let validator_account = Pallet::<T>::multilocation_to_account(&collator)?;
		let call =
			MantaCall::ParachainStaking(MantaParachainStakingCall::<T>::ExecuteDelegationRequest(
				delegator_account,
				validator_account,
			));
	...
	fn transfer_back(
		...
		// Construct xcm message.
		let call = MantaCall::Xtokens(MantaXtokensCall::<T>::Transfer(
	...
	fn charge_hosting_fee(
		...
		let vtoken = currency_id.to_vtoken().map_err(|_| Error::<T>::FailToConvert)?;

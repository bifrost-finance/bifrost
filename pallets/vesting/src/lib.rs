	...
	impl<T: Config> Pallet<T> {
		...
		pub fn set_vesting_per_block(
			...
			let schedules = Self::vesting(&target).ok_or(Error::<T>::NotVesting)?;
				...
				schedules[index].per_block() != per_block,
				Error::<T>::SamePerBlock
			);
			...
			let remained_vesting =
				schedules[index].locked_at::<T::BlockNumberToBalance>(now, Some(old_start_at));

			ensure!(
				remained_vesting <= schedules[index].locked(),
				Error::<T>::WrongLockedAmount
			);

			let mut new_start_offset = schedules[index].starting_block();
			if now > old_start_at {
			...
			Vesting::<T>::mutate_exists(&target, |info| {
				if let Some(ref mut vesting_info) = info {
					vesting_info[index].locked = remained_vesting;
					vesting_info[index].per_block = per_block;
					vesting_info[index].starting_block = new_start_offset;
				}
			});
...
impl<T: Config> Pallet<T> {
	...
	fn merge_vesting_info(
		...
		let locked = schedule1
			.locked_at::<T::BlockNumberToBalance>(now, schedule1_start_at)
			.saturating_add(
				schedule2.locked_at::<T::BlockNumberToBalance>(now, schedule2_start_at),
			);

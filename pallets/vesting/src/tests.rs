...
fn check_vesting_status() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				Vesting::vesting(&CHAR).unwrap().to_vec(),
				vec![user3_vesting_schedule]
			);
		});
}
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				...
				Some(ALICE_INIT_LOCKED - ALICE_PER_BLOCK)
			);
			assert_eq!(Vesting::vesting_balance(&BOB), Some(BOB_INIT_LOCKED));
			assert_eq!(Vesting::vesting_balance(&CHAR), Some(CHAR_INIT_LOCKED));
...
fn vested_transfer_should_work() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			let user1_vesting_schedule_1 =
				VestingInfo::new(ALICE_INIT_LOCKED, ALICE_PER_BLOCK, 0u64);
...
fn set_vesting_per_block_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(Vesting::set_vesting_per_block(
				RawOrigin::Root.into(),
				BOB,
				0,
				100
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let sched3_locked = sched2
				.locked_at::<Identity>(cur_block, Some(sched2.starting_block()))
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let sched2_locked = sched0
				.locked_at::<Identity>(cur_block, Some(sched0.starting_block()))
...
fn vesting_info_validate_works() {
	...
	assert_eq!(
		VestingInfo::new(min_transfer - 1, 1u64, 10u64).is_valid(),
		true
	);
	...
	assert_eq!(
		VestingInfo::new(min_transfer + 1, 0u64, 10u64).is_valid(),
		false
	);
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let sched = VestingInfo::new(
				<Test as Config>::MinVestedTransfer::get() as u64,
				1u64,
				10u64,
			);

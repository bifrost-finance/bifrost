...
fn set_cliff_should_work() {
	ExtBuilder::default().existential_deposit(ED).build().execute_with(|| {
		assert_ok!(Vesting::vest(Some(1).into()));
		assert_ok!(Vesting::force_set_cliff(RawOrigin::Root.into(), 1, 5));
...
fn set_vesting_per_block_should_work() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let user1_free_balance = Balances::free_balance(&1);
			assert_eq!(user1_free_balance, 10 * ED); // Account 1 has free balance
			...
			assert_eq!(Vesting::vesting_balance(&1), Some(5 * ED - 128));

			System::set_block_number(5);
			assert_eq!(System::block_number(), 5);

			// Account 1 has vested by half at the end of block 5
			assert_eq!(Vesting::vesting_balance(&1), Some(5 * ED - 5 * 128));

			// Change the per_block of account 1 to  256
			assert_ok!(Vesting::set_vesting_per_block(RawOrigin::Root.into(), 1, 0, 64));
			...
			let change1_user1_vesting_schedule = VestingInfo {
				locked: 5 * ED - 128 * 5,
				per_block: 64, // Vesting over 10 blocks
				starting_block: 4,
			};

			assert_eq!(Vesting::vesting(&1).unwrap(), vec![change1_user1_vesting_schedule]); // Account 1 has a vesting schedule
			assert_eq!(Vesting::vesting_balance(&1), Some(5 * ED - 128 * 5 - 2 * 64));

			assert_noop!(
				Vesting::set_vesting_per_block(RawOrigin::Root.into(), 1, 0, 64),
				Error::<Test>::SamePerBlock,
			);

			assert_ok!(Vesting::set_vesting_per_block(RawOrigin::Root.into(), 1, 0, 10));

			System::set_block_number(8);
			assert_eq!(System::block_number(), 8);

			let change2_user1_vesting_schedule = VestingInfo {
				locked: 5 * ED - 128 * 5 - 2 * 64,
				per_block: 10, // Vesting over 10 blocks
				starting_block: 5,
			};

			assert_eq!(Vesting::vesting(&1).unwrap(), vec![change2_user1_vesting_schedule]);
			assert_eq!(Vesting::vesting_balance(&1), Some(5 * ED - 128 * 5 - 2 * 64 - 3 * 10));

			System::set_block_number(57);
			assert_eq!(System::block_number(), 57);

			assert_noop!(
				Vesting::set_vesting_per_block(RawOrigin::Root.into(), 1, 0, 20),

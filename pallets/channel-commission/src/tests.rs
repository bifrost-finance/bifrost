...
fn remove_channel_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(ChannelCommission::remove_channel(
				RuntimeOrigin::signed(ALICE),
				0
			));
...
fn claim_commissions_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				Currencies::free_balance(KSM, &CHANNEL_A_RECEIVER),
				receiver_ksm_before + 100
			);

			// assure channel A's receiver's BNC balance is increased by 120
			assert_eq!(
				Currencies::free_balance(BNC, &CHANNEL_A_RECEIVER),
				receiver_bnc_before + 120
			);
		});
}
...
fn channel_commission_distribution_with_net_mint_positive_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				VtokenIssuanceSnapshots::<Runtime>::get(VKSM),
				(10000, 11000)
			);
			...
			assert_eq!(
				ChannelClaimableCommissions::<Runtime>::get(0, KSM),
				channel_a_commission
			);
			...
			assert_eq!(
				ChannelVtokenShares::<Runtime>::get(0, VKSM),
				channel_a_new_percentage
			);

			// check channel B has not been cleared yet
			assert_eq!(ChannelClaimableCommissions::<Runtime>::get(1, KSM), 0);
			assert_eq!(
				ChannelVtokenShares::<Runtime>::get(1, VKSM),
			...
			assert_eq!(
				ChannelClaimableCommissions::<Runtime>::get(1, KSM),
				channel_b_commission
			);
...
fn channel_commission_distribution_with_net_mint_negative_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				ChannelClaimableCommissions::<Runtime>::get(0, BNC),
				channel_a_commission
			);

			// check channel A vtoken share after being cleared
			assert_eq!(
				ChannelVtokenShares::<Runtime>::get(0, VBNC),
				channel_a_vtoken_share_before
			);

			// check channel B has not been cleared yet
			assert_eq!(ChannelClaimableCommissions::<Runtime>::get(1, BNC), 0);
			assert_eq!(
				ChannelVtokenShares::<Runtime>::get(1, VBNC),
				channel_b_vtoken_share_before
			);
			...
			assert_eq!(
				ChannelClaimableCommissions::<Runtime>::get(1, BNC),
				channel_b_commission
			);

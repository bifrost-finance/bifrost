...
impl<T: Config> OnRuntimeUpgrade for RemoveNonce<T> {
	fn on_runtime_upgrade() -> Weight {
		log::info!("RemoveNonce::on_runtime_upgrade execute, will remove nonce",);

		let mut weight: Weight = Weight::zero();

		// Remove the key type of CurrentNonce
		let module_prefix = CurrentNonce::<T>::module_prefix();
		let storage_prefix = CurrentNonce::<T>::storage_prefix();

		let old_data =
			storage_key_iter::<ChainId, Nonce, Blake2_128Concat>(module_prefix, storage_prefix)
				.drain()
				.collect::<sp_std::vec::Vec<_>>();

		for (key, _old_value) in old_data {

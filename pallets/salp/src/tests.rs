...
fn create_fund_under_non_signed_should_fail() {
	new_test_ext().execute_with(|| {
		assert_noop!(
			Salp::create(Origin::root(), 3_000, 1_000 * DOLLARS, 0, SlotLength::get() - 1,),
			DispatchError::BadOrigin,
		);

		assert_noop!(
			Salp::create(Origin::none(), 3_000, 1_000 * DOLLARS, 0, SlotLength::get() - 1,),
			DispatchError::BadOrigin,
		);
	});
}
...
fn create_fund_existed_should_fail() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			Salp::create(Some(ALICE).into(), 3_000, 1_000 * DOLLARS, 0, SlotLength::get() - 1,),
...
fn create_fund_first_slot_bigger_than_last_slot_should_fail() {
	new_test_ext().execute_with(|| {
		assert_noop!(
			Salp::create(Some(ALICE).into(), 3_000, 1_000 * DOLLARS, SlotLength::get() - 1, 0),
...
#[test]
fn contribute_with_wrong_par_id_should_fail() {
	new_test_ext().execute_with(|| {
		assert_ok!(Salp::create(
			Some(ALICE).into(),
			3_000,

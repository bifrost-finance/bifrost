...
fn set_fund_end_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Salp::confirm_withdraw(Some(ALICE).into(), 3_000, true, CONTRIBUTON_INDEX));
...
fn withdraw_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Salp::confirm_withdraw(Some(ALICE).into(), 4_000, true, CONTRIBUTON_INDEX));
...
fn withdraw_when_xcm_error_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Salp::fund_retire(Some(ALICE).into(), 3_000));
		assert_ok!(Salp::withdraw(Some(ALICE).into(), 3_000));
		assert_ok!(Salp::confirm_withdraw(Some(ALICE).into(), 3_000, false, CONTRIBUTON_INDEX));
		...
		assert_ok!(Salp::fund_fail(Some(ALICE).into(), 4_000));
		assert_ok!(Salp::withdraw(Some(ALICE).into(), 4_000));
		assert_ok!(Salp::confirm_withdraw(Some(ALICE).into(), 4_000, false, CONTRIBUTON_INDEX));
...
fn refund_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Salp::confirm_refund(Some(ALICE).into(), BRUCE, 3_000, true, CONTRIBUTON_INDEX));
...
fn confirm_refund_without_enough_reserved_should_fail() {
	new_test_ext().execute_with(|| {
		...
		// ```
		let result =
			Salp::confirm_refund(Some(ALICE).into(), BRUCE, 3_000, true, CONTRIBUTON_INDEX);
...
fn refund_with_wrong_origin_should_fail() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Salp::refund(Some(BRUCE).into(), 3_000));
		assert_noop!(
			Salp::confirm_refund(Origin::root(), BRUCE, 3_000, true, CONTRIBUTON_INDEX),
			DispatchError::BadOrigin
		);
		assert_noop!(
			Salp::confirm_refund(Origin::none(), BRUCE, 3_000, true, CONTRIBUTON_INDEX),
			DispatchError::BadOrigin
		);
		assert_noop!(
			Salp::confirm_refund(Some(BRUCE).into(), BRUCE, 3_000, true, CONTRIBUTON_INDEX),
			DispatchError::BadOrigin
		);
	});
}
...
fn redeem_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Salp::confirm_redeem(Some(ALICE).into(), BRUCE, 3_000, true, CONTRIBUTON_INDEX));
		...
		assert_ok!(Salp::confirm_redeem(Some(ALICE).into(), CATHI, 3_000, true, CONTRIBUTON_INDEX));
...
fn confirm_redeem_when_not_in_redeeming_should_fail() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			Salp::confirm_redeem(Some(ALICE).into(), BRUCE, 3_000, true, CONTRIBUTON_INDEX),
...
fn redeem_with_wrong_origin_should_fail() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			Salp::confirm_redeem(Origin::none(), BRUCE, 3_000, true, CONTRIBUTON_INDEX),
			DispatchError::BadOrigin
		);
	});
}
...
fn redeem_with_expired_vsbond_should_fail() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(Salp::redeem(Some(BRUCE).into(), 3_000, 50), Error::<Test>::VSBondExpired);

		assert_noop!(Salp::redeem(Some(CATHI).into(), 3_000, 50), Error::<Test>::VSBondExpired);
	});
}

...
fn get_swap_output_amount() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(StablePool::mint_inner(&3, 0, amounts, 0));
			assert_eq!(
				StablePool::get_swap_output(0, 0, 1, 5000000u128).ok(),
				Some(4999301)
			);
			assert_noop!(
...
fn redeem_movr() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				Tokens::free_balance(pool_asset, &0),
				321765598211330627258732
			);
			assert_ok!(StablePool::redeem_proportion_inner(
				&0,
				0,
				million_unit(300_000),
				vec![0, 0]
			));
			assert_eq!(
				Tokens::free_balance(pool_asset, &0),
				21765598211330627258732
			);

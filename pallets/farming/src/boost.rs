...
pub(crate) trait BoostInterface<AccountId, CurrencyId, Balance, BlockNumber> {
	fn charge_boost_reward_pool(
		who: &AccountId,
		rewards: Vec<(CurrencyId, Balance)>,
	) -> DispatchResult;
	fn vote(who: &AccountId, vote_list: Vec<(PoolId, Percent)>) -> DispatchResult;
	fn refresh_vebnc_farming(who: &AccountId) -> DispatchResult;
	fn start_boost_round(who: &AccountId, round_length: BlockNumber) -> DispatchResult;
}

impl<T: Config> BoostInterface<AccountIdOf<T>, CurrencyIdOf<T>, BalanceOf<T>, BlockNumberFor<T>>
	for Pallet<T>
{
	// type BoostPoolInfo = BoostPoolInfo<CurrencyIdOf<T>, BalanceOf<T>, T::BlockNumber>;
	fn start_boost_round(who: &AccountIdOf<T>, round_length: BlockNumberFor<T>) -> DispatchResult {
	...
	fn vote(who: &AccountIdOf<T>, vote_list: Vec<(PoolId, Percent)>) -> DispatchResult {
...
impl<T: Config> Pallet<T> {
	...
	pub(crate) fn auto_start_boost_round() {
		let mut boost_pool_info = Self::boost_pool_infos();
		if !boost_pool_info.next_round_whitelist.is_empty() {
			boost_pool_info.whitelist = boost_pool_info.next_round_whitelist.clone();

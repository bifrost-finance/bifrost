...
use node_primitives::{CurrencyId, RewardTrait, TokenSymbol};
...
fn query_vtoken_should_be_ok() {
	new_test_ext().execute_with(|| {
		common();
		let (vdot_id, vksm_id, referer_one, referer_two, staking_profit) = (
...
fn dispatch_reward_is_be_ok() {
	new_test_ext().execute_with(|| {
		// Condition initial
		common();
		let (vdot_id, veth_id, referer_one, referer_two, staking_profit) = (
			CurrencyId::VToken(TokenSymbol::DOT),
		...
		// The first query asset
		let referer_one_assets =
			<Test as Config>::CurrenciesHandler::free_balance(vdot_id, &referer_one);
		assert_eq!(0, referer_one_assets);
		let referer_two_assets =
			<Test as Config>::CurrenciesHandler::free_balance(vdot_id, &referer_two);
...
fn more_than_256_dispatch_reward_is_be_ok() {
	new_test_ext().execute_with(|| {
		// Condition initial
		let (vdot_id, referer_one, referer_two, staking_profit) =
		...
		let referer_254_assets =
			<Test as Config>::CurrenciesHandler::free_balance(vdot_id, &(referer_two + 255));

...
fn exchange_for_token_should_work() {
	...
	.execute_with(|| {
		...
		assert_eq!(ksm_pool, BancorPool { currency_id: CurrencyId::Token(TokenSymbol::KSM), token_pool: 0, vstoken_pool: 0, token_ceiling: 0, token_base_supply: 2 * VSKSM_BASE_SUPPLY, vstoken_base_supply: VSKSM_BASE_SUPPLY });
		assert_eq!(dot_pool, BancorPool { currency_id: CurrencyId::Token(TokenSymbol::DOT), token_pool: 0, vstoken_pool: 0, token_ceiling: 0, token_base_supply: 2 * VSDOT_BASE_SUPPLY, vstoken_base_supply: VSDOT_BASE_SUPPLY });
		...
		assert_noop!(Bancor::exchange_for_token(Origin::signed(ALICE), DOT, 50, 50), Error::<Test>::PriceNotQualified);

		// exchange succeeds.
		assert_ok!(Bancor::exchange_for_token(Origin::signed(ALICE), DOT, 50, 48)); 
		let dot_pool = Bancor::get_bancor_pool(DOT).unwrap();
		assert_eq!(dot_pool, BancorPool { currency_id: CurrencyId::Token(TokenSymbol::DOT), token_pool: 49, vstoken_pool: 50, token_ceiling: 51, token_base_supply: 2 * VSDOT_BASE_SUPPLY, vstoken_base_supply: VSDOT_BASE_SUPPLY });
...
fn exchange_for_vstoken_should_work() {
	...
	.execute_with(|| {
		...
		BancorPools::<Test>::insert(DOT, updated_pool);
		let dot_pool = Bancor::get_bancor_pool(DOT).unwrap();
		assert_eq!(dot_pool, BancorPool { currency_id: CurrencyId::Token(TokenSymbol::DOT), token_pool: 50, vstoken_pool: 50, token_ceiling: 0, token_base_supply: 2 * VSDOT_BASE_SUPPLY, vstoken_base_supply: VSDOT_BASE_SUPPLY });

		// exchange rate is lower than the specified slippage(49 <50), which leads to exchange failure.
		assert_noop!(Bancor::exchange_for_vstoken(Origin::signed(ALICE), DOT, 49, 50), Error::<Test>::PriceNotQualified);

		// exchange succeeds.
		assert_ok!(Bancor::exchange_for_vstoken(Origin::signed(ALICE), DOT, 49, 48));
		let dot_pool = Bancor::get_bancor_pool(DOT).unwrap();
		assert_eq!(dot_pool, BancorPool { currency_id: CurrencyId::Token(TokenSymbol::DOT), token_pool: 1, vstoken_pool: 1, token_ceiling: 0, token_base_supply: 2 * VSDOT_BASE_SUPPLY, vstoken_base_supply: VSDOT_BASE_SUPPLY });
...
fn add_token_should_work() {
	...
	.execute_with(|| {
		...
		let dot_pool = Bancor::get_bancor_pool(DOT).unwrap();
		assert_eq!(dot_pool, BancorPool { currency_id: CurrencyId::Token(TokenSymbol::DOT), token_pool: 0, vstoken_pool: 0, token_ceiling: 20000, token_base_supply: 2 * VSDOT_BASE_SUPPLY, vstoken_base_supply: VSDOT_BASE_SUPPLY });
		...
		let dot_pool = Bancor::get_bancor_pool(DOT).unwrap();
		assert_eq!(dot_pool, BancorPool { currency_id: CurrencyId::Token(TokenSymbol::DOT), token_pool: price, vstoken_pool: 20000, token_ceiling: 20000-price, token_base_supply: 2 * VSDOT_BASE_SUPPLY, vstoken_base_supply: VSDOT_BASE_SUPPLY });
		...
		let dot_pool = Bancor::get_bancor_pool(DOT).unwrap();
		assert_eq!(dot_pool, BancorPool { currency_id: CurrencyId::Token(TokenSymbol::DOT), token_pool: 14641-50, vstoken_pool: 20000 - price, token_ceiling: 5359 + 50, token_base_supply: 2 * VSDOT_BASE_SUPPLY, vstoken_base_supply: VSDOT_BASE_SUPPLY });
	});
}
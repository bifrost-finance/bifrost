...
parameter_types! {
	...
	pub VsdotPerSecond: (AssetId, u128) = (
		MultiLocation::new(
			1,
			X2(Parachain(SelfParaId::get()), GeneralKey(CurrencyId::VSToken(TokenSymbol::DOT).encode()))
		).into(),
		dot_per_second()
	);
	...
	pub AcaPerSecond: (AssetId, u128) = (
		MultiLocation::new(
			1,
			X2(Parachain(parachains::polkadot::acala::ID), GeneralKey(parachains::polkadot::acala::ACA_KEY.to_vec()))
		).into(),
		// ACA:DOT = 10:1
		dot_per_second() * 10
	);
	pub AusdPerSecond: (AssetId, u128) = (
		MultiLocation::new(
			1,
			X2(Parachain(parachains::polkadot::acala::ID), GeneralKey(parachains::polkadot::acala::AUSD_KEY.to_vec()))
		).into(),
		// aUSD:DOT = 40:1
		dot_per_second() * 40
	);
...
pub type Trader = (
	...
	FixedRateOfFungible<VsdotPerSecond, ToTreasury>,
...
parameter_types! {
	pub const RelayChainTokenSymbol: TokenSymbol = TokenSymbol::DOT;

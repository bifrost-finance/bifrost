...
impl OnRuntimeUpgrade for XcmInterfaceMigration {
	...
	fn pre_upgrade() -> Result<Vec<u8>, &'static str> {
		let xcm_interface_xcm_dest_weight_and_fee_cnt =
			XcmInterface::XcmDestWeightAndFee::<Runtime>::iter().count();
		...
		let slp_xcm_dest_weight_and_fee_cnt = Slp::XcmDestWeightAndFee::<Runtime>::iter().count();
		log::info!(
			target: LOG_TARGET,
	...
	fn post_upgrade(cnt: Vec<u8>) -> Result<(), &'static str> {
		...
		let new_xcm_interface_xcm_weight_and_fee_cnt =
			XcmInterface::XcmWeightAndFee::<Runtime>::iter().count();

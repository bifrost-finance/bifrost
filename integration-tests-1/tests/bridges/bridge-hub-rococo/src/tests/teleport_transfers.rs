...
fn send_asset_from_bifrost_polkadot_to_bifrost_kusama(id: MultiLocation, amount: u128) {
	...
	BifrostPolkadot::execute_with(|| {
		assert_ok!(
			pallet_xcm::Pallet::<bifrost_polkadot_runtime::Runtime>::execute(
				signed_origin,
		...
		assert_ok!(
			pallet_xcm::Pallet::<bifrost_polkadot_runtime::Runtime>::send_xcm(
				Here,
				destination,
				// bx!(beneficiary.into()),
				// bx!(assets.into()),
		...
		type Balances = <BifrostPolkadot as BifrostPolkadotPallet>::Balances;
...
#[test]
fn teleport_bnc_from_bifrost_polkadot_to_bifrost_kusama() {
	...
	BifrostPolkadot::execute_with(|| {
		...
		type Balances = <BifrostPolkadot as BifrostPolkadotPallet>::Balances;
		println!("Balances: {:?}", Balances::total_issuance());
		...
		// assert_ok!(AssetRegistry::register_multilocation(sudo_origin.clone(), BNC, bx!(bnc_at_bifrost_polkadot.into()), Weight::default()));

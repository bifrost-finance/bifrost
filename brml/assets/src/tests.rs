...
fn issuing_asset_units_to_issuer_should_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            Assets::token_details(dot_type),
            Token::new(b"DOT".to_vec(), 12, 10000)
        );
        assert_eq!(Assets::account_assets((dot_type, alice)).balance, 10000);

        assert_ok!(Assets::issue(Origin::root(), vdot_type, alice, 20000));
        assert_eq!(
            Assets::token_details(vdot_type),
            Token::new(b"vDOT".to_vec(), 12, 20000)
        );
        ...
        assert_eq!(
            Assets::token_details(ausd_type),
            Token::new(b"aUSD".to_vec(), 18, 50000)
        );
        ...
        assert_eq!(
            ...
            vec![
                EventRecord {
                    phase: Phase::Initialization,
                    event: TestEvent::assets(RawEvent::Issued(TokenSymbol::DOT, 1, 10000)),
                    topics: vec![]
                },
                EventRecord {
                    phase: Phase::Initialization,
                    event: TestEvent::assets(RawEvent::Issued(TokenSymbol::vDOT, 1, 20000)),
                    topics: vec![]
                },
                EventRecord {
                    phase: Phase::Initialization,
                    event: TestEvent::assets(RawEvent::Issued(TokenSymbol::aUSD, 2, 20000)),
                    topics: vec![]
                },
                EventRecord {
                    phase: Phase::Initialization,
                    event: TestEvent::assets(RawEvent::Issued(TokenSymbol::aUSD, 2, 30000)),
                    topics: vec![]
                },
            ]
        );
...
fn transferring_amount_above_available_balance_should_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            Assets::token_details(ausd_type),
            Token::new(b"aUSD".to_vec(), 18, 10000)
        );
        ...
        assert_eq!(
            ...
            vec![
                EventRecord {
                    phase: Phase::Initialization,
                    event: TestEvent::assets(RawEvent::Issued(ausd_type, 1, 10000)),
                    topics: vec![]
                },
                EventRecord {
                    phase: Phase::Initialization,
                    event: TestEvent::assets(RawEvent::Transferred(ausd_type, 1, 2, 1000)),
                    topics: vec![]
                }
            ]
        );
...
fn destroying_asset_balance_with_positive_balance_should_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            Assets::token_details(ausd_type),
            Token::new(b"aUSD".to_vec(), 18, 9000)
        );
        ...
        assert_eq!(
            ...
            vec![
                EventRecord {
                    phase: Phase::Initialization,
                    event: TestEvent::assets(RawEvent::Created(
                        0,
                        Token {
                ...
                EventRecord {
                    phase: Phase::Initialization,
                    event: TestEvent::assets(RawEvent::Destroyed(ausd_type, 1, 1000)),
                    topics: vec![]
                }
            ]
        );

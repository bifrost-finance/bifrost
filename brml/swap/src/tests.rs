...
fn swap_vtoken_to_token_should_be_ok() {
	new_test_ext().execute_with(|| {
		...
		// set fee
		let rate = 3;
		assert_ok!(Swap::set_exchange(Origin::ROOT, token_id, vtoken_id, rate));
		assert_eq!(<ExchangeRate<Test>>::get(token_id, vtoken_id), rate);
...
fn exchange_token_to_vtoken_should_be_ok() {
	new_test_ext().execute_with(|| {
		...
		// issue token to bob
		let bob_token_issued = 60;
		assert_ok!(assets::Module::<Test>::issue(Origin::ROOT, token_id, bob, bob_token_issued));
		...
		assert_ok!(Swap::exchange_token_to_vtoken(Origin::signed(bob), bob_token_exchange, token_id, vtoken_id));
		assert_eq!(<assets::Balances<Test>>::get((token_id, bob)), bob_token_issued - bob_token_exchange); // check bob's token change
		assert_eq!(<assets::Balances<Test>>::get((vtoken_id, bob)), bob_token_exchange * rate); // check bob's token change
	});
}
...
fn exchange_vtoken_to_token_should_be_ok() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(assets::Module::<Test>::issue(Origin::ROOT, vtoken_id, bob, bob_vtoken_issued));
		...
		assert_ok!(Swap::exchange_vtoken_to_token(Origin::signed(bob), bob_vtoken_exchange, vtoken_id, token_id));
		assert_eq!(<assets::Balances<Test>>::get((vtoken_id, bob)), bob_vtoken_issued - bob_vtoken_exchange); // check bob's token change
		assert_eq!(<assets::Balances<Test>>::get((token_id, bob)), bob_vtoken_exchange / rate); // check bob's token change
	});
}

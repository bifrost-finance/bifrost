...
impl<T: Trait> Module<T> {
    ...
    #[allow(dead_code)]
    pub(crate) fn single_liquidity_charged_fee(
        balance: T::Balance,
        token_weight: T::PoolWeight,
        total_weight: T::PoolWeight,
        swap_fee: T::Fee,
    ) -> (T::Balance, T::Balance) {
        let charged_fee = {
            ...
            let swap_fee: T::Balance = swap_fee.into();
            let proportion =
                balance.saturating_mul(total_weight.saturating_sub(token_weight)) / total_weight;
    ...
    pub(crate) fn value_function(
        pool: &[(T::AssetId, T::Balance, T::PoolWeight)],
    ) -> Result<T::InvariantValue, Error<T>> {
    ...
    pub(crate) fn calculate_spot_price(
        ...
        let token_weight_in =
            TryInto::<u128>::try_into(token_weight_in).map_err(|_| Error::<T>::ConvertFailure)?;
        let token_balance_out =
            TryInto::<u128>::try_into(token_balance_out).map_err(|_| Error::<T>::ConvertFailure)?;
        let token_weight_out =
            TryInto::<u128>::try_into(token_weight_out).map_err(|_| Error::<T>::ConvertFailure)?;
        let swap_fee =
        ...
        let fixed_price = token_balance_in.saturating_mul(token_weight_out)
            / (token_weight_in
                .saturating_mul(token_balance_out)
    ...
    pub(crate) fn calculate_single_out_given_pool_in(
        ...
        let swap_fee =
            TryInto::<u128>::try_into(swap_fee).map_err(|_| Error::<T>::ConvertFailure)?;
        let exit_fee =
            TryInto::<u128>::try_into(exit_fee).map_err(|_| Error::<T>::ConvertFailure)?;
        // to fixed num
        ...
        let weight_ratio = Self::weight_ratio(token_weight_in, token_total_weight)?;
        let base = {
            let upper = pool_supply.saturating_sub(
                pool_amount_in
                    .saturating_mul(FixedI128::<extra::U64>::from_num(1).saturating_sub(exit_fee)),
            );

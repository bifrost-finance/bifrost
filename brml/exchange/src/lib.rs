...
decl_storage! {
	trait Store for Module<T: Trait> as Exchange {
		...
		ExchangeRate get(fn exchange_rate): linked_map hasher(blake2_256) T::AssetId => (T::AssetId, T::ExchangeRate);
		/// change rate per block, vtoken => (token, rate_per_block)
		RatePerBlock get(fn rate_per_block): linked_map hasher(blake2_256) T::AssetId => (T::AssetId, T::RatePerBlock);
	}
}

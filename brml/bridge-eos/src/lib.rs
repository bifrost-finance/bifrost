...
impl<T: Trait> Module<T> {
	#[cfg(feature = "std")]
	fn verify_block_headers(mut init_merkle: IncrementalMerkle,
						block_headers: Vec<SignedBlockHeader>,
						block_merkle_roots: Vec<Checksum256>,
						block_ids_list: Vec<Vec<Checksum256>>
	) -> Result<(), Error>
	{
		...
		for (index, (block_header, expected_mroot)) in block_headers.iter().zip(block_merkle_roots.iter()).enumerate() {
			Self::verify_signature(block_header, expected_mroot)?;

			Self::verify_merkle_root(index, &mut init_merkle, &block_header, &block_ids_list, &expected_mroot)?;
		}
		Ok(())
	}

	#[cfg(feature = "std")]
	fn verify_signature(block_header: &SignedBlockHeader,
	...
	) -> Result<(), Error>
	{
		...
		ensure!(block_header.verify(*expected_mroot, ps_hash, pk).is_ok(), Error::SignatureVerificationFailure);
		Ok(())
	}

	#[cfg(feature = "std")]
	fn verify_merkle_root(index: usize,
	...
	) -> Result<(), Error>
	{
		...
		for id in block_ids {
			ensure!(init_merkle.append(*id).is_ok(), Error::IncreMerkleError);
		}

		ensure!(init_merkle.append(block_header.block_header.previous).is_ok(), Error::IncreMerkleError);
		let block_id = block_header.id().map_err(|_| Error::GetBlockIdFailure)?;
		ensure!(init_merkle.append(block_id).is_ok(), Error::IncreMerkleError);

		let actual_mroot = init_merkle.get_root();
		ensure!(actual_mroot == *expected_mroot, Error::MerkleRootVerificationFailure);
		Ok(())
	}
}
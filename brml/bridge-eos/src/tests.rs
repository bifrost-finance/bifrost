...
use eos_chain::{
    get_proof, Action, ActionReceipt, Checksum256, IncrementalMerkle, ProducerAuthoritySchedule,
...
fn get_producer_schedules_should_work() {
    new_test_ext().execute_with(|| {
        ...
        let schedule = signed_blocks_headers
            .first()
            .as_ref()
            .unwrap()
            .block_header
        ...
        ProducerSchedules::insert(
            schedule.version,
            (&schedule.producers, schedule_hash.unwrap()),
        );
...
fn verify_block_header_signature_should_succeed() {
    new_test_ext().execute_with(|| {
        ...
        let schedule_hash_and_producer_schedule = BridgeEos::get_schedule_hash_and_public_key(
            signed_blocks_headers[0].block_header.new_producers.as_ref(),
        );
...
fn verify_block_headers_should_succeed() {
    new_test_ext().execute_with(|| {
        ...
        let block_ids_list: Vec<Vec<Checksum256>> = block_ids_list
            ...
            .map(|ids| {
                ids.iter()
                    .map(|id| Checksum256::from_str(id).unwrap())
                    .collect::<Vec<_>>()
            })
            .collect::<Vec<_>>();
...
fn change_schedule_should_work() {
    new_test_ext().execute_with(|| {
        ...
        let v1_producers: Result<ProducerSchedule, _> =
            serde_json::from_str(&v1_producers_str.unwrap());
        ...
        ProducerSchedules::insert(
            v1_producers.version,
            (&v1_producers.producers, v1_schedule_hash.unwrap()),
        );
...
fn prove_action_should_be_ok() {
    new_test_ext().execute_with(|| {
        ...
        let v2_producers: Result<ProducerSchedule, _> =
            serde_json::from_str(&v2_producers_str.unwrap());
        ...
        ProducerSchedules::insert(
            v2_producers.version,
            (&v2_producers.producers, v2_schedule_hash.unwrap()),
        );
...
fn bridge_eos_genesis_config_should_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            ProducerSchedules::get(version),
            (producers, schedule_hash.unwrap())
        );
    });
}
...
fn bridge_tx_report() -> dispatch::DispatchResult {
    ...
    #[allow(deprecated)]
    BridgeEos::pre_dispatch(&crate::Call::bridge_tx_report(tx_outs.clone()))
        .map_err(|e| <&'static str>::from(e))?;

    BridgeEos::bridge_tx_report(Origin::none(), tx_outs)
}
...
fn lite_json_deserialize_push_transaction() {
    ...
    assert_eq!(
        trx_id.unwrap(),
        "58e71de1c3f1a93417addbf1fc79e58e4f57a0930ec9c4f294b4ad64375c9dc6"
    );
}

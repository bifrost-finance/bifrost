        ...
        match self {
            TxOut::Generated(mut multi_sig_tx) => {
                ...
                let trx = Transaction::read(&multi_sig_tx.raw_tx, &mut 0)
                    .map_err(|_| Error::<T>::EosChainError)?;
                let sig: Signature = trx
                    .sign(sk, chain_id.clone())
                    .map_err(|_| Error::<T>::EosChainError)?;
                let sig_hex_data = sig
                    .to_serialize_data()
                    .map_err(|_| Error::<T>::EosChainError)?;

                multi_sig_tx.multi_sig.signatures.push(TxSig {
                    author,
...
pub(crate) mod eos_rpc {
    ...
    pub(crate) fn get_block<T: crate::Trait>(
        node_url: &str,
    ...
    pub(crate) fn get_transaction_id<T: crate::Trait>(
        ...
        let splited_strs: Vec<&str> = trx_response
            .trim_matches(|c| c == '{' || c == '}')
            .split("processed")
            .collect();

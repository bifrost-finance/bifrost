...
use crate::Error;
use eos_chain::{Action, Asset, Checksum256, Read, SerializeData, Signature, SignedTransaction, Transaction};
...
impl<AccountId: PartialEq + Clone> TxOut<AccountId> {
	...
	pub fn generate<T: crate::Trait>(self, eos_node_url: &str) -> Result<Self, Error<T>> {
		match self {
			TxOut::Initial(mut multi_sig_tx) => {
				...
				// fetch info
				let info: GetInfo = get_info().fetch(&hyper_client).map_err(|_| Error::<T>::EOSRpcError)?;
				let chain_id: Vec<u8> = hex::decode(info.chain_id).map_err(|_| Error::<T>::EosChainError)?;
				let head_block_id = info.head_block_id;

				// fetch block
				let block: GetBlock = get_block(head_block_id).fetch(&hyper_client).map_err(|_| Error::<T>::EOSRpcError)?;
	...
	#[cfg(feature = "std")]
	pub fn send<T: crate::Trait>(self, eos_node_url: &str) -> Result<TxOut<AccountId>, Error<T>> {
		match self {
			TxOut::Signed(multi_sig_tx) => {
				...
				let signatures = multi_sig_tx.multi_sig.signatures.iter()
					.map(|tx_sig|
						Signature::read(&tx_sig.signature, &mut 0).map_err(|_| Error::<T>::EOSRpcError)
					)
					.map(Result::unwrap)
					.collect::<Vec<Signature>>();
				let trx = Transaction::read(&multi_sig_tx.raw_tx, &mut 0).map_err(|_| Error::<T>::EOSRpcError)?;
				let signed_trx = SignedTransaction {
					signatures,
					context_free_data: vec![],
					trx,
				};
				let push_tx: PushTransaction = push_transaction(signed_trx).fetch(&hyper_client).map_err(|_| Error::<T>::EOSRpcError)?;
//				let tx_id = hex::decode(push_tx.transaction_id).map_err(|_| Error::<T>::InvalidChecksum256)?;
				let tx_id = Checksum256::from_str(&push_tx.transaction_id).map_err(|_| Error::<T>::InvalidChecksum256)?;

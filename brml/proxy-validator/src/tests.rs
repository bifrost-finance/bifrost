...
fn stake_should_ok() {
    new_test_ext().execute_with(|| {
        ...
        let target = 1;
        let amount = 100;
        assert_ok!(ProxyValidator::stake(
            Origin::root(),
            token_symbol,
            target,
            amount
...
fn unstake_should_ok() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(ProxyValidator::unstake(
            Origin::root(),
            token_symbol,
            target,
            unstake_amount
...
fn register_should_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            ProxyValidator::validators(token_symbol, origin_id),
            validator
        );
    });
}
...
fn set_need_amount_should_work() {
    new_test_ext().execute_with(|| {
        ...
        let new_need = 2000;
        assert_ok!(ProxyValidator::set_need_amount(
            origin,
            token_symbol,
            new_need
        ));
...
fn set_reward_per_block_should_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(ProxyValidator::set_reward_per_block(
            origin,
            token_symbol,
            reward_per_block
        ));
...
fn deposit_should_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            ...
            Token::new(b"DOT".to_vec(), 8, 10000)
        );
        ...
        assert_ok!(ProxyValidator::deposit(
            origin.clone(),
            dot_type,
            deposit_amount
        ));

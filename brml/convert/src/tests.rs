...
fn update_rate_multiple_times() {
    new_test_ext().execute_with(|| {
        ...
        let convert_rate = 20;
        assert_ok!(Convert::set_convert_price(
            Origin::root(),
            ausd_type,
            convert_rate
        ));
        let update_rate = 2;
        assert_ok!(Convert::set_price_per_block(
            Origin::root(),
            ausd_type,
            update_rate
        ));
        ...
        // 20 - 2 * 3 = 14
        assert_eq!(
            Convert::convert_price(ausd_type),
            convert_rate - update_rate * change_times
        );
    });
}
...
fn to_vtoken_should_be_ok() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            ...
            bob_dot_issued - bob_dot_convert
        ); // check bob's token change
        assert_eq!(
            <assets::AccountAssets<Test>>::get((vdot_type, bob)).balance,
            bob_vdot_issued + bob_dot_convert * rate
        ); // check bob's token change

        assert_eq!(
            Convert::pool(dot_type),
            ConvertPool::new(bob_dot_convert, bob_dot_convert * rate)
        );
    });
}
...
fn to_token_should_be_ok() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            <assets::AccountAssets<Test>>::get((vdot_type, bob)).balance,
            bob_vdot_issued - bob_vdot_convert
        ); // check bob's token change
        assert_eq!(
            <assets::AccountAssets<Test>>::get((dot_type, bob)).balance,
            bob_dot_issued + bob_vdot_convert / rate
        ); // check bob's token change
...
fn add_new_refer_channel_should_be_ok() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Convert::to_vtoken(
            ...
            vdot_type,
            bob_dot_convert1.0,
            Some(bob_dot_convert1.1)
        ));
        assert_ok!(Convert::to_vtoken(
            Origin::signed(bob),
            vdot_type,
            bob_dot_convert2.0,
            Some(bob_dot_convert2.1)
        ));
        assert_ok!(Convert::to_vtoken(
            Origin::signed(bob),
            vdot_type,
            bob_dot_convert2.0,
            Some(bob_dot_convert2.1)
        )); // recommend referer2 2 times
        assert_ok!(Convert::to_vtoken(
            Origin::signed(bob),
            vdot_type,
            bob_dot_convert3.0,
            Some(bob_dot_convert3.1)
        ));
        assert_ok!(Convert::to_vtoken(
            Origin::signed(bob),
            vdot_type,
        ...
        assert_eq!(
            ...
            (
                vec![
                    ...
                    (referer2, bob_dot_convert2.0 * 2 * rate),
        ...
        assert_ok!(Convert::to_vtoken(
            Origin::signed(alice),
            vdot_type,
            alice_dot_convert1.0,
            Some(alice_dot_convert1.1)
        ));
        assert_ok!(Convert::to_vtoken(
            Origin::signed(alice),
            vdot_type,
            alice_dot_convert2.0,
            Some(alice_dot_convert2.1)
        ));
        assert_ok!(Convert::to_vtoken(
            Origin::signed(alice),
            vdot_type,
            alice_dot_convert3.0,
            None
        )); // no referer
        ...
        assert_eq!(
            ...
            alice_vdot_issued
                + rate * (alice_dot_convert1.0 + alice_dot_convert2.0 + alice_dot_convert3.0)
        );
        ...
        assert_eq!(
            ...
            (
                vec![
                    (referer2, alice_dot_convert1.0 * rate),
        ...
        let all_channels: BTreeMap<u64, u64> = [
            ...
            (
                referer2,
                (bob_dot_convert2.0 * 2 + alice_dot_convert1.0) * rate,
            ),
            (referer3, bob_dot_convert3.0 * rate),
            (referer4, alice_dot_convert2.0 * rate),
        ]
        ...
        let all_channels: BTreeMap<u64, u64> = [
            ...
            (
                referer2,
                (bob_dot_convert2.0 * 2 + alice_dot_convert1.0) * rate - alice_vdot,
            ),
            (referer3, bob_dot_convert3.0 * rate),
            (referer4, alice_dot_convert2.0 * rate),
        ]
